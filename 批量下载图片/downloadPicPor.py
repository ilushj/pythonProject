...
for row in range(3, sheet.max_row + 1):
    ...
    # 处理其他列信息
    # 处理其他列信息
    department = sheet.cell(row=row, column=headers.get('部门', 2)).value or "Unknown"
    date = sheet.cell(row=row, column=headers.get('日期', 3)).value or "Unknown"
    time = sheet.cell(row=row, column=headers.get('时间', 4)).value or "Unknown"
